...
import {assertEqual as eq, I, B, T, K} from '../util/util';
...
describe('Algebra', function (){
  ...
  describe('ap', function (){
    ...
    property('composition using map', _mx, _mf, _mf, function (mx, mf, mg){
      return eq(ap(ap(map(B, mg), mf), mx), ap(mg, ap(mf, mx)));
    });
  ...
  describe('resolve', function (){
    ...
    property('identity for ap', _mx, function (mx){
      return eq(ap(resolve(I), mx), mx);
    });

    property('homomorphism with ap', _x, function (x){
      return eq(ap(resolve(f), resolve(x)), resolve(f(x)));
    });

    property('interchange with ap', _x, _mf, function (x, mf){
      return eq(ap(mf, resolve(x)), ap(resolve(T(x)), mf));
    });
